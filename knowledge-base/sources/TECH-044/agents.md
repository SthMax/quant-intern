...
Powered by the open-source code-interpreter service (self-hosted)
...
Within an attached workspace, Agents can list directories, read files, search text, author files, and run Bash through the selected worker. A per-Agent Git name and email can identify commits made there without supplying repository credentials. Deployments can limit new personal workers with statefulCodeSessions.principalWorkers.
When an attached worker advertises multiple workspace roots, the chat composer requires one selection for each attached environment used by the Agent or its Subagents. The conversation retains those selections across turns and approval resumes; LibreChat validates them against the live worker and blocks sending if a required workspace is missing or unavailable. A worker that advertises exactly one workspace is selected automatically for a new conversation. See Attached environments and pairing for details and current limitations.
...
Configure MCP servers in your librechat.yaml file
Restart LibreChat to initialize the connections
Create or edit an agent
Open Add tools in the Agent Builder and select MCP
Select the MCP server(s) you want to add; each server appears as a single entry
...
Open the Agent Builder and add MCP tools
Click the dropdown on any MCP tool
Toggle "Defer Loading" — deferred tools show a clock icon
...
Enable both programmatic_tools and execute_code in the librechat.yaml agents capabilities.
Enable Code Interpreter on the agent.
Add MCP tools, expand an MCP server, and toggle Programmatic for individual tools or for the full server. Programmatic tools show a code icon.
...
Administrators can also opt in to owner-scoped cancellation with endpoints.agents.backgroundTasks.ordinaryToolCancellation: true. The task reports that cancellation was requested until the underlying invocation settles, and its capacity remains reserved during that interval. Cancellation is cooperative: a tool that ignores the abort signal may still complete successfully. The control applies to ordinary process-local background tools, including attached Bash, but not independently owned detached Subagents.
...
OpenAI Image Tools: Image generation & editing using GPT-Image-1
Gemini Image Tools: Image generation and image-context editing using Gemini image models
DALL-E-3: Image generation from text descriptions
Stable Diffusion / Flux: Text-to-image generation
Wolfram: Computational and mathematical capabilities
OpenWeather: Weather data retrieval
...
Azure AI Search: Information retrieval from Azure AI Search indexes
Traversaal: A robust search API for LLM Agents
Create an Agent with Image Tools
Add the image tool credentials to .env, such as IMAGE_GEN_OAI_API_KEY for OpenAI Image Tools.
Restart LibreChat so the new environment variables are loaded.
Select Agents from the endpoint menu.
Open the Agent Builder from the side panel and create or edit an agent.
Open the agent's Tools list, select OpenAI Image Tools, Gemini Image Tools, DALL-E-3, Stable Diffusion, or Flux, then save the agent.
Start a chat with that agent and ask it to generate or edit an image.
...
Upload as Text (requires context capability)
...
See Upload as Text
Upload for File Search (requires file_search capability, toggled ON)
...
Upload for Code Interpreter (requires execute_code capability, toggled ON)
...
Image Upload: For visual content the agent can reference
File Search Upload: Documents for RAG capabilities
Code Interpreter Upload: Files for code processing
File Context: Documents with extracted text to supplement agent instructions
...
Trusted services can also manage Agents, attached files, and Skills through the beta Agent Management API. Administrators bind each OIDC machine client to one existing LibreChat user and tenant; every request inherits that user's current role capabilities and resource ACLs rather than receiving separate administrative access.
Access Roles
...
Click the Share button in the footer (visible when you're the author, an admin, or have been granted SHARE permission on that specific agent)
Search for users, groups, or roles in the people picker and assign each a role
Optionally toggle Public to make the agent visible to everyone on the instance (requires the SHARE_PUBLIC feature permission)
...
Configure desired capabilities (Code Interpreter, File Search, File Context, etc.)
...
On this page
Agents: Build Custom AI AssistantsGetting StartedTool LibraryModel ConfigurationVersion HistoryAgent CapabilitiesCode InterpreterStateful Code SessionsFile SearchFile ContextModel Context Protocol (MCP)Agents with MCP ToolsManaging MCP ToolsDeferred ToolsActivity GroupsLive Reasoning LabelsTool Intent LabelsProgrammatic Tool CallingBackground Tool CallsAsk User and Tool ApprovalSkillsMemoryArtifactsToolsCreate an Agent with Image ToolsActionsHandoffsAgent ChainSubagentsAdvanced SettingsMax Agent StepsFile ManagementIn Chat InterfaceIn Agent BuilderSharing and PermissionsAccess RolesSharing an AgentAdministrator ControlsUser-Level SharingSteering and Queued MessagesNotesAgent Event DeliveryOptional ConfigurationBest PracticesRecapMigration Required (v0.8.0-rc3+)What the Migration DoesRunning the Migration1. For the default docker-compose.yml (if you use docker compose up to start the app):2. For the deploy-compose.yml3. For local development (from project root):What Happens During MigrationAgents API (Beta)What's next?
