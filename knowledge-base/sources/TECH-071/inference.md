Part 7 of [How To Scale Your Model](/scaling-book) ([Part 6: Training LLaMA](../applied-training) | [Part 8: Serving LLaMA](../applied-inference))
...
![](/scaling-book/assets/img/naive-inference-1400.webp)
...
![](/scaling-book/assets/img/cached-inference-1400.webp)

Figure: diagram of efficient Transformer sampling with a KV cache. Prefill processes our prompt and saves all the per-token key-value activations in a cache. Generation takes this cache (and the last-token logits), samples a new token, and passes that new token through the model, attending to the KV cache and saving the new token's key-value projections back to the cache. This is an $O(n)$ algorithm in the MLP block.
...
So far we’ve mostly treated a Transformer as a stack of feedforward blocks. While this is often reasonable from a FLOPs and memory standpoint, it’s not sufficient to properly model inference.
...
As we saw in [Part 4](../transformers), the major components of a Transformer forward pass are:
...
![](/scaling-book/assets/img/latency-cost-1400.webp)
...
| --- | --- | --- |
| FFW params | d\_model <sup>2</sup> x ffw\_multiplier x 3 (for SwiGLU gate, up, and down projections) x n\_layers | 5,120 x 5,120 x 2.7 x 3 x 40 = **8.5e9** |
...
![](/scaling-book/assets/img/gmqa-1400.webp)
...
![](/scaling-book/assets/img/kv-sharing-1400.webp)

Left: Multiple layers of pure global attention. Right: An example of some global/local interleaving pattern with sharing with adjacent layers. Source: Character.ai blog.
...
![](/scaling-book/assets/img/paged-attention-1400.webp)

Figure: during generation, a single token ("forth") attends to multiple KV cache blocks/pages. By paging the KV cache, we avoid loading or storing more memory than we need to. Taken from the PagedAttention paper.
...
![](/scaling-book/assets/img/esta-figure-1400.webp)
...
![](/scaling-book/assets/img/batched-prefill-1400.webp)
...
![](/scaling-book/assets/img/interleaving-1400.webp)
...
![](/scaling-book/assets/img/disaggregation-1400.webp)
...
![](/scaling-book/assets/img/continuous-batching-1400.webp)
...
![](/scaling-book/assets/img/prefix-caching-trie-1400.webp)

Figure: KV prefix cache implemented as an LRU trie. We can avoid duplicating KV memory by sharing prefixes. Source: Character.ai blog.
...
![](/scaling-book/assets/img/batch-scaling-latency-1400.webp)
...
![](/scaling-book/assets/img/batch-scaling-throughput-1400.webp)
...
![](/scaling-book/assets/img/2d-weight-stationary-1400.webp)
...
![](/scaling-book/assets/img/all-gather-1400.webp)
...
![](/scaling-book/assets/img/spec-sampling1-1400.webp)
...
![](/scaling-book/assets/img/spec-sampling2-1400.webp)
...
![](/scaling-book/assets/img/spec-sampling3-1400.webp)
