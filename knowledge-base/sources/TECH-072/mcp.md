...
Hermes identifies itself with a [Client ID Metadata Document](about:/docs/reference/mcp-config-reference#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.
...
See [OAuth over SSH / Remote Hosts](about:/docs/guides/oauth-over-ssh#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id` / `client_secret`, scope customization, and re-auth via `hermes mcp login <server>`.
