The Multimodal Intelligence of Muse Spark 1.2 | Meta AI Research
Skip to main contentAI Research
AI Developers
Try Meta AI
...
Organize cosmeticsPack lunchFind duckFind food
Organize cosmetics
Muse Spark breaks down a long-horizon task “arrange objects (cosmetics) in the organizer” into actionable steps. In this example, the low-level action is done by a VLA model based on an experimental version of a smaller Muse family model.
Muse Spark is used as a planner to complete long-horizon manipulation tasks: “pack the fruits in the lunch box” and “put utensils in the plate.” In this example, the low-level action is done by a VLA model based on an experimental version of a smaller Muse family model.
Muse Spark parses multimodal observations and calls tools to guide a robot to navigate in an unstructured environment to find a rubber duck.
In a simulated environment, Muse Spark leverages diverse tools, following a complex human instruction to find chocolate and bring it back to the coffee table. The grasps are simplified.
